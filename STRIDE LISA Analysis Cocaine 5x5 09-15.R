...
{
stride <- read.csv("C:/Users/gkfrj/Documents/R/STRIDE_Raw.csv")
...
stride <- stride %>% as_tibble()

# Cocaine Data
#### I selected only cocaine data from the STRIDE data set.
cocaine.derivatives <- str_to_upper(
  str_split("Cocaine, Crack, Cocaine Powder, Cocaine Base, Cocaine precursor, Coca Leaves, Cocaine metabolite", ", ")[[1]])
cocaine <- stride[stride$Drug %in% cocaine.derivatives,]
cocaine$Drug <- as.character(cocaine$Drug)
cocaine$Drug <- as.factor(cocaine$Drug)
}

cocaine
cocaine %>% filter(MethAcq=="S") %>% group_by(State, Seize.Year) %>% summarise(months=n())
cocaine %>% filter(State=="AK", Seize.Year==2004)
cocaine.agg <- cocaine %>% filter(MethAcq=="S") %>% group_by(State, Seize.Year, Seize.Month) %>% summarise(Nt.Wt=sum(Nt.Wt))
cocaine.agg %>% group_by(State, Seize.Year) %>% summarise(months=n()) -> cocaine.agg.n
cocaine.agg.n %>% filter(months == 12) %>% as.data.frame
table((cocaine.agg.n %>% filter(months == 12))$State)
sort(table((cocaine.agg.n %>% filter(months == 12))$State), decreasing=T)
...
for (state_name in unique(LISA$state)) {
  ...
  n_increase <- sum(N[c(1,3,5), c(2,4,5)])-N[5,5]
  n_decrease <- sum(N[c(2,4,5), c(1,3,5)])-N[5,5]
  diffusion_count$increase <- n_increase
  diffusion_count$decrease <- n_decrease
  diffusion_count$non_adj_inc <- (sum(N[c(1,5),c(2,4,5)])-N[5,5])
  diffusion_count$adj_inc <- sum(N[3,c(2,4,5)])
  diffusion_count$non_adj_dec <- (sum(N[c(4,5),c(1,3,5)])-N[5,5])
  diffusion_count$adj_dec <- sum(N[2,c(1,3,5)])
...
# write.csv(diffusion_summary, "STRIDE cocaine diffusion summary 5x5 (count).csv", row.names=F)
...
for (i in 1:47) {
  ...
  adj_inc_m <- ifelse(LISA_C_t0 == 3 & LISA_C_t1 %in% c(0, 1, 4), 1, 0) %>% sum
  non_adj_inc_m <- (ifelse(LISA_C_t0 %in% c(0,2) & LISA_C_t1 %in% c(0, 1, 4), 1, 0) %>% sum) - n_stationary_insig
  adj_dec_m <- ifelse(LISA_C_t0 == 4 & LISA_C_t1 %in% c(0, 2, 3), 1, 0) %>% sum
  non_adj_dec_m <- (ifelse(LISA_C_t0 %in% c(0,1) & LISA_C_t1 %in% c(0, 2, 3), 1, 0) %>% sum) - n_stationary_insig
