...
cocaine <- read.csv("cocaine all count HIDTA (12-20-2022).csv") %>% as_tibble
...
# write.csv(relative.MoransI, "crack count KNN5 R codes 999 (relative).csv", row.names=F)
...
# write.csv(absolute.b1.MoransI, "crack count KNN5 R codes 999 (absolute base 1).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "crack count KNN5 R codes 999 (absolute base t_1).csv", row.names=F)

# crack Plots
LISA.rel <- read.csv("crack count KNN5 R codes 999 (relative).csv") %>% as_tibble
LISA.abs.1 <- read.csv("crack count KNN5 R codes 999 (absolute base 1).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("crack count KNN5 R codes 999 (absolute base t_1).csv") %>% as_tibble
...
LISA_C.rel.pnorm <- relative.MoransI[,c(1:3, grep("LISA_C", names(relative.MoransI)))]
LISA_C.rel.punif <- relative.MoransI[,c(1:3, grep("LISA_C", names(relative.MoransI)))]
LISA_C.rel.pnorm2 <- relative.MoransI[,c(1:3, grep("LISA_C", names(relative.MoransI)))]
LISA_C.rel.punif2 <- relative.MoransI[,c(1:3, grep("LISA_C", names(relative.MoransI)))]

LISA_C.rel[, -(1:3)] %>% flatten %>% unlist %>% table # two tails pseudo p-value
LISA_C.rel.pnorm[, -(1:3)] %>% flatten %>% unlist %>% table # two tails pnorm
LISA_C.rel.punif[, -(1:3)] %>% flatten %>% unlist %>% table # two tails punif
LISA_C.rel.pnorm2[, -(1:3)] %>% flatten %>% unlist %>% table # upper tail pnorm
LISA_C.rel.punif2[, -(1:3)] %>% flatten %>% unlist %>% table # upper tail punif
# no pseudo p-value for upper tail

LISA_C.abs.1[, -(1:3)] %>% flatten %>% unlist %>% table
...
LISA.rel.map <- LISA.rel.map %>% 
  pivot_longer(c(-long, -lat, -GEOID, -state, -county), names_to="Month_Year", values_to="seizure_counts") %>% 
...
for (year in 2020:2021) { # monthly maps of county-level overdose count
  LISA.rel.map %>% filter(Year == year) %>% 
    ggplot(mapping = aes(long, lat, group = GEOID, fill=overdose)) +
    geom_polygon(color = "#FFFFFF", size = .05) +
    facet_wrap(. ~ format(parse_date(Month_Year, "%b_%Y"), "%b_%Y")) +
    scale_fill_viridis_c() +
    labs(fill = "Overdose Count") +
    ggtitle("County-level All Drugs") -> overdose_map
  ggsave(paste("county-level all drugs overdose_", year, ".jpg", sep=""), overdose_map, width=50, height=30, units="cm")
  ...
  LISA.rel.map %>% filter(Year == year) %>% 
    ...
    ggtitle("County-level All Drugs") -> overdose_per_pop_map
  ggsave(paste("county-level all drugs overdose_per_pop_", year, ".jpg", sep=""), overdose_per_pop_map, width=50, height=30, units="cm")
}


for (year in 2018:2021) { # monthly maps of seziure counts
  LISA.rel.map %>% filter(grepl(year, Month_Year)) %>% 
    ggplot(mapping = aes(long, lat, group = GEOID, fill=seizure_counts)) +
    geom_polygon(color = "#FFFFFF", size = .05) +
    facet_wrap(. ~ Month_Year) +
    # coord_equal() +
    scale_fill_viridis_c() +
    labs(fill = "Seizure Counts") -> seizure_counts_map
  ggsave(paste("crack seizure_counts_", year, ".jpg", sep=""), seizure_counts_map, width=50, height=30, units="cm")
  
  LISA.rel.map %>% filter(grepl(year, Month_Year)) %>% 
    ggplot(mapping = aes(long, lat, group = GEOID, fill=counts_per_pop)) +
    geom_polygon(color = "#FFFFFF", size = .05) +
    facet_wrap(. ~ Month_Year) +
    # coord_equal() +
    scale_fill_viridis_c() +
    labs(fill = "Counts Per Pop.") -> counts_per_pop.map
  ggsave(paste("crack counts_per_pop_", year, ".jpg", sep=""), counts_per_pop.map, width=50, height=30, units="cm")
}


cbind(LISA.rel[LISA.rel %>% select(Jan_2018:Dec_2018) %>% apply(2, which.max), 1:2],
      counts=LISA.rel %>% select(Jan_2018:Dec_2018) %>% apply(2, max))
...
for (year in 2018:2021) { # monthly maps of LISA_C.rel
  ...
  ggsave(paste("Crack_Count_LISA_C_rel_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs1
  ...
  ggsave(paste("Crack_Count_LISA_C_abs1_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs_t1
  ...
  ggsave(paste("Crack_Count_LISA_C_abs_t1_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}
...
# write.csv(relative.MoransI, "cocaine all counts KNN5 R codes 999 (relative).csv", row.names=F)
...
# write.csv(absolute.b1.MoransI, "cocaine all counts KNN5 R codes 999 (absolute base 1).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "cocaine all counts KNN5 R codes 999 (absolute base t_1).csv", row.names=F)

# Cocaine Plots
LISA.rel <- read.csv("cocaine all counts KNN5 R codes 999 (relative).csv") %>% as_tibble
LISA.abs.1 <- read.csv("cocaine all counts KNN5 R codes 999 (absolute base 1).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("cocaine all counts KNN5 R codes 999 (absolute base t_1).csv") %>% as_tibble
...
for (year in 2018:2021) { # monthly maps of seziure counts
  LISA.rel.map %>% filter(grepl(year, Month_Year)) %>% 
    ...
    scale_fill_viridis_c() +
    labs(fill = "Seizure Counts") -> seizure_counts_map
  ggsave(paste("cocaine seizure_counts_", year, ".jpg", sep=""), seizure_counts_map, width=50, height=30, units="cm")
  ...
  LISA.rel.map %>% filter(grepl(year, Month_Year)) %>% 
    ...
    scale_fill_viridis_c() +
    labs(fill = "Counts Per Pop.") -> counts_per_pop.map
  ggsave(paste("cocaine counts_per_pop_", year, ".jpg", sep=""), counts_per_pop.map, width=50, height=30, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.rel
  LISA_C.rel.map %>% filter(substr(Month_Year,10,10) == substr(year,4,4)) %>% 
    ...
    scale_fill_manual(values = c("Insig"="grey90",
                                 ...
                                 "HH"="red")) +
    labs(fill = "LISA_C_rel") -> LISA_C_map
  ggsave(paste("cocaine_all_counts_LISA_C_rel_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs1
  LISA_C.abs1.map %>% filter(substr(Month_Year,10,10) == substr(year,4,4)) %>% 
    ...
    scale_fill_manual(values = c("Insig"="grey90",
                                 ...
                                 "HH"="red")) +
    labs(fill = "LISA_C_abs1") -> LISA_C_map
  ggsave(paste("cocaine_all_counts_LISA_C_abs1_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs_t1
  LISA_C.abs_t1.map %>% filter(substr(Month_Year,10,10) == substr(year,4,4)) %>% 
    ...
    scale_fill_manual(values = c("Insig"="grey90",
                                 ...
                                 "HH"="red")) +
    labs(fill = "LISA_C_abs_t1") -> LISA_C_map
  ggsave(paste("cocaine_all_counts_LISA_C_abs_t1_", year, ".jpg", sep=""), LISA_C_map, width=50, height=30, units="cm")
}