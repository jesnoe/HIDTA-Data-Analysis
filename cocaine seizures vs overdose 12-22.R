...
cocaine_count <- read.csv("cocaine all count HIDTA (12-20-2022).csv") %>% as_tibble
...
# all cocaine
# {
#   cocaine <- seizures %>% filter(Drug %in% c("Cocaine", "Crack", "Cocaine, Powder", "Cocaine Base",
...
# cocaine <- cocaine %>% filter(!is.na(GEOID) & !is.na(county))
# cocaine # 1099 counties in total.
# write.csv(cocaine, "cocaine all count HIDTA (12-20-2022).csv", row.names=F)
