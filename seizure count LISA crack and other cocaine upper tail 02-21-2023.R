...
# write.csv(relative.MoransI, "crack counts KNN5 R codes 999 upper tail relative (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.b1.MoransI, "crack counts KNN5 R codes 999 upper tail absolute base_1 (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "crack counts KNN5 R codes 999 upper tail absolute base_t1 (02-21-2023).csv", row.names=F)

# crack Plots
LISA.rel <- read.csv("crack counts KNN5 R codes 999 upper tail relative (02-21-2023).csv") %>% as_tibble
LISA.abs.1 <- read.csv("crack counts KNN5 R codes 999 upper tail absolute base_1 (02-21-2023).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("crack counts KNN5 R codes 999 upper tail absolute base_t1 (02-21-2023).csv") %>% as_tibble
...
LISA.rel.two_sided <- read.csv("crack count KNN5 R codes 999 two-sided relative (02-21-2023).csv") %>% as_tibble
...
overdose_map <- overdose_filtered %>% 
...
LISA.rel.2020 <- left_join(counties.obs[, c(1:2,6:7,10,12)],
                           LISA.rel %>%
                             select(GEOID, Jan_2020:Dec_2020) %>%
                             mutate(seizure_counts=Jan_2020+Feb_2020+Mar_2020+Apr_2020+May_2020+Jun_2020+Jul_2020+Aug_2020+Sep_2020+Oct_2020+Nov_2020+Dec_2020) %>% 
                             select(GEOID, seizure_counts),
                           by = "GEOID") %>% 
  mutate(Year=2020)
LISA.rel.2020 <- left_join(LISA.rel.2020, populations[, -(1:2)], by=c("GEOID", "Year"))
LISA.rel.2020$counts_per_pop <- LISA.rel.2020$seizure_counts/LISA.rel.2020$population
overdose_map <- overdose_filtered %>%
  rename(overdose=Provisional.Drug.Overdose.Deaths) %>% 
  group_by(GEOID, Year) %>%
  summarise(missing=ifelse(sum(is.na(overdose)) == 12, 1, 0),
            overdose=ifelse(missing, NA, sum(overdose, na.rm=T)))
LISA.rel.2020 <- left_join(LISA.rel.2020, overdose_map, by=c("GEOID", "Year"))
LISA.rel.2020$overdose_per_pop <- LISA.rel.2020$overdose/LISA.rel.2020$population
...
LISA.rel.2020 %>%
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> overdose_map
# ggsave(paste("county-level all drugs overdose_2020.jpg", sep=""), overdose_map, width=20, height=15, units="cm")
...
LISA.rel.2020 %>%
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> overdose_per_pop_map
# ggsave(paste("county-level all drugs overdose_per_pop_2020.jpg", sep=""), overdose_per_pop_map, width=22, height=15, units="cm")
...
# ggsave("crack n_HH annual (rel).jpg", n_HH_map, width=20, height=15, units="cm")
...
# ggsave("crack n_HH 2018-2021 (rel).jpg", n_HH_map_all, width=20, height=15, units="cm")
...
# write.csv(relative.MoransI, "cocaine other counts KNN5 R codes 999 upper tail relative (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.b1.MoransI, "cocaine other counts KNN5 R codes 999 upper tail absolute base_1 (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "cocaine other counts KNN5 R codes 999 upper tail absolute base_t1 (02-21-2023).csv", row.names=F)

# Cocaine Plots
LISA.rel <- read.csv("cocaine other counts KNN5 R codes 999 upper tail relative (02-21-2023).csv") %>% as_tibble
LISA.abs.1 <- read.csv("cocaine other counts KNN5 R codes 999 upper tail absolute base_1 (02-21-2023).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("cocaine other counts KNN5 R codes 999 upper tail absolute base_t1 (02-21-2023).csv") %>% as_tibble
...
LISA_n_HH.map %>% pivot_longer(-c(long, lat, group, GEOID, state, county, n_HH_All), names_to="year", values_to="n_HH_year") %>% 
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> n_HH_map
# ggsave("other cocaine n_HH annual (rel).jpg", n_HH_map, width=20, height=15, units="cm")
...
LISA_n_HH.map %>% 
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> n_HH_map_all
# ggsave("other cocaine n_HH 2018-2021 (rel).jpg", n_HH_map_all, width=20, height=15, units="cm")
...
# Drug seizure map in 2020
for (drug in c("crack", "cocaine other", "meth", "marijuana")) {
  LISA.rel <- read.csv(paste(drug, "counts KNN5 R codes 999 upper tail relative (02-21-2023).csv")) %>% as_tibble
  LISA.rel.2020 <- left_join(counties.obs[, c(1:2,6:7,10,12)],
                             LISA.rel %>%
                               select(GEOID, Jan_2020:Dec_2020) %>%
                               mutate(seizure_counts=Jan_2020+Feb_2020+Mar_2020+Apr_2020+May_2020+Jun_2020+Jul_2020+Aug_2020+Sep_2020+Oct_2020+Nov_2020+Dec_2020) %>% 
  ...
  LISA.rel.2020 %>%
    ...
    geom_polygon(color = "#000000", size = .05) +
    scale_fill_viridis_c(na.value="white") +
    labs(fill = "Seizure Count") +
    ggtitle(drug) + 
    theme_bw() + 
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> seizure_map
  ggsave(paste(drug, "seizure counts_2020.jpg"), seizure_map, width=20, height=15, units="cm")
  
  LISA.rel.2020 %>%
    ggplot(mapping = aes(long, lat, group = group, fill=counts_per_pop)) +
    geom_polygon(color = "#000000", size = .05) +
    scale_fill_viridis_c(na.value="white") +
    labs(fill = "Seizure Count/Pop.") +
    ggtitle(drug) + 
    theme_bw() + 
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> seizure_map
  ggsave(paste(drug, "seizure counts_per_pop_2020.jpg"), seizure_map, width=20, height=15, units="cm")
}
