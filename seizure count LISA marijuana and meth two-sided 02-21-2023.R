...
for (i in 1:48) {
  ...
  localM.month <- localmoran_abs(seizure.marijuana, nb.obj.marijuana, nsim=nperm, zero.policy=T, xx=NULL, alternative="two.sided")
...
# write.csv(relative.MoransI, "marijuana counts KNN5 R codes 999 two-sided relative (02-21-2023).csv", row.names=F)
...
for (i in 1:48) {
  seizure.marijuana <- t(seizures.marijuana)[i,]
  localM.month <- localmoran_abs(seizure.marijuana, nb.obj.marijuana, nsim=nperm, zero.policy=T, xx=x.bar_p, alternative="two.sided")
  localM.month$LISA_C <- ifelse(localM.month$quadr=="High-High", 1,
                                ifelse(localM.month$quadr=="Low-Low", 2,
                                       ifelse(localM.month$quadr=="Low-High", 3, 4)))
  localM.month$LISA_C <- ifelse(localM.month$`Pr(folded) Sim` <= alpha, localM.month$LISA_C, 0)
...
# write.csv(absolute.b1.MoransI, "marijuana counts KNN5 R codes 999 two-sided absolute base_1 (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "marijuana counts KNN5 R codes 999 two-sided absolute base_t1 (02-21-2023).csv", row.names=F)

# marijuana Plots
LISA.rel <- read.csv("marijuana counts KNN5 R codes 999 two-sided relative (02-21-2023).csv") %>% as_tibble
LISA.abs.1 <- read.csv("marijuana counts KNN5 R codes 999 two-sided absolute base_1 (02-21-2023).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("marijuana counts KNN5 R codes 999 two-sided absolute base_t1 (02-21-2023).csv") %>% as_tibble
...
# ggsave("marijuana  n_HL annual (rel).jpg", n_HL_map, width=20, height=15, units="cm")

LISA_n_HL.map %>% 
  ggplot(mapping = aes(long, lat, group = group, fill=n_HL_All)) +
  geom_polygon(color = "#000000", size = .05) +
  scale_fill_viridis_c(na.value="white") +
  labs(fill = "# of HL", title="# of HL 2018-2021 (marijuana)") + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> n_HL_map_all
# ggsave("marijuana  n_HL 2018-2021 (rel).jpg", n_HL_map_all, width=20, height=15, units="cm")
...
for (year in 2018:2021) { # monthly maps of LISA_C.rel
  ...
  ggsave(paste("marijuana_Count_LISA_C_rel_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs1
  LISA_C.abs1.map %>% filter(year(Month_Year) == year & month(Month_Year) %in% 1:4) %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> LISA_C_map
  ggsave(paste("marijuana_Count_LISA_C_abs1_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs_t1
  LISA_C.abs_t1.map %>% filter(year(Month_Year) == year & month(Month_Year) %in% 1:4) %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> LISA_C_map
  ggsave(paste("marijuana_Count_LISA_C_abs_t1_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}
...
for (i in 1:48) {
  ...
  localM.month <- localmoran_abs(seizure.meth, nb.obj.meth, nsim=nperm, zero.policy=T, xx=NULL, alternative="two.sided")
  localM.month$LISA_C <- ifelse(localM.month$quadr=="High-High", 1,
                                ifelse(localM.month$quadr=="Low-Low", 2,
                                       ifelse(localM.month$quadr=="Low-High", 3, 4)))
  localM.month$LISA_C <- ifelse(localM.month$`Pr(folded) Sim` <= alpha, localM.month$LISA_C, 0)
...
# write.csv(relative.MoransI, "meth counts KNN5 R codes 999 two-sided relative (02-21-2023).csv", row.names=F)
...
for (i in 1:48) {
  seizure.meth <- t(seizures.meth)[i,]
  localM.month <- localmoran_abs(seizure.meth, nb.obj.meth, nsim=nperm, zero.policy=T, xx=x.bar_p, alternative="two.sided")
  localM.month$LISA_C <- ifelse(localM.month$quadr=="High-High", 1,
                                ifelse(localM.month$quadr=="Low-Low", 2,
                                       ifelse(localM.month$quadr=="Low-High", 3, 4)))
  localM.month$LISA_C <- ifelse(localM.month$`Pr(folded) Sim` <= alpha, localM.month$LISA_C, 0)
...
# write.csv(absolute.b1.MoransI, "meth counts KNN5 R codes 999 two-sided absolute base_1 (02-21-2023).csv", row.names=F)
...
# write.csv(absolute.t1.MoransI, "meth counts KNN5 R codes 999 two-sided absolute base_t1 (02-21-2023).csv", row.names=F)

# meth Plots
LISA.rel <- read.csv("meth counts KNN5 R codes 999 two-sided relative (02-21-2023).csv") %>% as_tibble
LISA.abs.1 <- read.csv("meth counts KNN5 R codes 999 two-sided absolute base_1 (02-21-2023).csv") %>% as_tibble
LISA.abs.t_1 <- read.csv("meth counts KNN5 R codes 999 two-sided absolute base_t1 (02-21-2023).csv") %>% as_tibble
...
LISA_n_HL.map %>% pivot_longer(-c(long, lat, group, GEOID, state, county, n_HL_All), names_to="year", values_to="n_HL_year") %>% 
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> n_HL_map
# ggsave("meth  n_HL annual (rel).jpg", n_HL_map, width=20, height=15, units="cm")

LISA_n_HL.map %>% 
  ggplot(mapping = aes(long, lat, group = group, fill=n_HL_All)) +
  geom_polygon(color = "#000000", size = .05) +
  scale_fill_viridis_c(na.value="white") +
  labs(fill = "# of HL", title="# of HL 2018-2021 (meth)") + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) -> n_HL_map_all
# ggsave("meth  n_HL 2018-2021 (rel).jpg", n_HL_map_all, width=20, height=15, units="cm")
...
for (year in 2018:2021) { # monthly maps of LISA_C.rel
  LISA_C.rel.map %>% filter(year(Month_Year) == year & month(Month_Year) %in% 1:4) %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> LISA_C_map
  ggsave(paste("meth_counts_LISA_C_rel_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs1
  LISA_C.abs1.map %>% filter(year(Month_Year) == year & month(Month_Year) %in% 1:4) %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> LISA_C_map
  ggsave(paste("meth_counts_LISA_C_abs1_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}
...
for (year in 2018:2021) { # monthly maps of LISA_C.abs_t1
  LISA_C.abs_t1.map %>% filter(year(Month_Year) == year & month(Month_Year) %in% 1:4) %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) -> LISA_C_map
  ggsave(paste("meth_counts_LISA_C_abs_t1_", year, " two-sided (4 months).jpg", sep=""), LISA_C_map, width=20, height=15, units="cm")
}