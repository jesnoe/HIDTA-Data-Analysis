...
seizures <- LISA3[, 21:68]

absolute.MoransI <- LISA3
...
for (i in 1:48) {
  ...
  x.bar_p <- mean(as.data.frame(LISA3)[,21+i-1])
  print(paste(i,"complete"))
}
LISA3[,72:82]; absolute.MoransI[,72:82]

# write.csv(absolute.MoransI, "CountyKNN3 R codes 999 (relative).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 9999 (relative).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 99999 (relative).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 999 (absolute base t_1).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 9999 (absolute).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 99999 (absolute base 1).csv", row.names=F)
# write.csv(absolute.MoransI, "CountyKNN3 R codes 99999 (absolute base t_1).csv", row.names=F)
